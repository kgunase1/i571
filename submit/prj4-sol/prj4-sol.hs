...
categoryItems [] _ = []
categoryItems (item:rest) categoryItem
  | itemCategory item == categoryItem = item : categoryItems rest categoryItem
  | otherwise = categoryItems rest categoryItem
...
comprCategoryItems items categoryItem = [item | item <- items, itemCategory item == categoryItem]
...
assoc _ [] = Nothing
assoc key ((k, v):rest) = if key == k then Just v else assoc key rest
...
sortBindings bindings = sortBy compareBindings bindings
  where
    compareBindings :: Binding -> Binding -> Ordering
    compareBindings (varName1, _) (varName2, _) = compare varName1 varName2
...
substTerm (Var varName) bindings =
  case assoc varName bindings of
